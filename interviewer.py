...
- Следующий вопрос ОБЯЗАТЕЛЬНО по target_topic_group из Strategy, но вопрос НЕ должен повторять предыдущие. Если дано уточнение (instruction) — строго следуй ему; при "не знаю"/затрудняется — задай более простой вопрос или дай короткую подсказку, затем спроси снова.
...
Последние вопросы (Используй их ТОЛЬКО для того, чтобы не задавать один и тот же вопрос несколько раз.):
"{recent}"
